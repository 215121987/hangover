jQuery(function (a) {
    ...
    if ($fragment_refresh = {url:wc_cart_fragments_params.ajax_url, type:"POST", data:{action:"woocommerce_get_refreshed_fragments"}, success:function (b) {
        ...
        try {
            var c = a.parseJSON(sessionStorage.getItem(wc_cart_fragments_params.fragment_name)), d = sessionStorage.getItem("wc_cart_hash"), e = a.cookie("hangover_cart_hash");
...
function showCartItemCount(b){
    $(".cart-holder > h3 > .cart-items").html($(b).find("li").not(".empty").length);
}