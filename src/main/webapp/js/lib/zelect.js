...
(function($) {
    ...
    $.fn.zelect = function(opts) {
        ...
        return this.each(function() {
            ...
            function appendItem(item, term) {
                var itm = renderContent($('<li>').data('zelect-item', item), opts.renderItem(item, term));
                $.each(item, function(key, value){
                    if(key != 'value' && key != 'label' && key != 'selected'){
                        itm.attr(key, value);
                    }
                });
                $list.append(itm);
    ...
    function selectBased($select, $list, regexpMatcher, appendItemFn) {
        ...
        function itemFromOption($option) {
            ...
            c['label']= $option.text();
            return c;//{ value: $option.attr('value'), label: $option.text() }
        }
